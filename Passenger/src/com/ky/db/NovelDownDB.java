...
/**
 * 
 * 建一个小说下载的下载库，正在下载的视频，在这里面会有记录
 * */
public class NovelDownDB extends SQLiteOpenHelper {

	public final static String DATABASE_NAME = "VIDEODOWN.db";
	...
	public NovelDownDB(Context context) {
	...
	public void onCreate(SQLiteDatabase db) {
		...
		+ " INTEGER primary key autoincrement, " + n_URL + " text," + n_NAME
				+ " text);";
	...
	public long select(String name) {
		...
		if (cursor != null && cursor.getCount() > 0) {
			for (cursor.moveToFirst(); !cursor.isAfterLast(); cursor
					.moveToNext()) {
				time = cursor.getLong(cursor.getColumnIndex(n_URL));
			}
			return time;
		} else {
			return 0;
	...
	public long insert(String m_URL, String m_NAME)
