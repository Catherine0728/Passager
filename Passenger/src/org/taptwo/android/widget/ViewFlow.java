...
public class ViewFlow extends AdapterView<Adapter> {
	...
	public boolean onInterceptTouchEvent(MotionEvent ev) {
		...
		case MotionEvent.ACTION_DOWN:
			System.out.println("event to down");
			/*
		...
		case MotionEvent.ACTION_MOVE:
			System.out.println("event to move");
		...
		case MotionEvent.ACTION_UP:
			System.out.println("event to up");
			if (mTouchState == TOUCH_STATE_SCROLLING) {
	...
	public boolean onTouchEvent(MotionEvent ev) {
		...
		switch (action) {
		case MotionEvent.ACTION_DOWN:
			System.out.println("event to down2");
			/*
			 * If being flinged and user touches, stop the fling. isFinished
			 * will be false if being flinged.
			 */
		...
		case MotionEvent.ACTION_MOVE:
			...
			if (mTouchState == TOUCH_STATE_SCROLLING) {
				...
				final int scrollX = getScrollX();
				if (deltaX < 0) {
					System.out.println("event to move2《");
					if (scrollX > 0) {
						System.out.println("event to move2《》" + scrollX
								+ "------" + deltaX);
						// scrollBy(Math.max(-scrollX, deltaX), 0);
						scrollBy(-scrollX, 0);
					}
		...
		case MotionEvent.ACTION_UP:
			System.out.println("event to up2");
			if (mTouchState == TOUCH_STATE_SCROLLING) {
				final VelocityTracker velocityTracker = mVelocityTracker;
				velocityTracker.computeCurrentVelocity(1000, mMaximumVelocity);
				int velocityX = (int) velocityTracker.getXVelocity();
