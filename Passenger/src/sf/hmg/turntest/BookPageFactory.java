...
public class BookPageFactory {
	...
	private int m_backColor = 0xFFe1bd84; // 背景颜色
	...
	public BookPageFactory(int w, int h) {
		...
		mVisibleHeight = mHeight - marginHeight-40 ;
...
//		long lLen = book_file.length();
		byte[] data = strFilePath.getBytes();
		m_mbBufLen = data.length;
		m_mbBuf = ByteBuffer.wrap(data);
//		m_mbBufLen = (int) lLen;
//		m_mbBuf = new RandomAccessFile(book_file, "r").getChannel().map(
//				FileChannel.MapMode.READ_ONLY, 0, lLen);
	}

	public void openString(ArrayList<String> list) throws IOException {
		// book_file = new File(strFilePath);
		long lLen = list.size();
		m_mbBufLen = (int) lLen;
		m_mbBuf = new RandomAccessFile(book_file, "r").getChannel().map(
